...
export const moveToColumnCell = (cardNotation, index) => (dispatch, getState) => {
  ...
  return dispatch({type: MOVE_TO_COLUMN_CELL, payload: {subStack}})

};

export const moveToFreeCell = (cardNotation, index) => (dispatch, getState) => {
  const state = getState();

  const isEmpty = isCellEmpty(state, {cellType: CELL_TYPES.FREE_CELL, cellIndex: index});
  if (!isEmpty) {
  ...
  return dispatch({type: MOVE_TO_FREE_CELL, payload: {subStack}});
};
